...
export class CampaignTourFormComponent implements OnInit {
  ...
  loadEquipment(): void {
    ...
    this.selectedTours.forEach((t) => {
      this.tourAuthoringService
        .getEquipmentForTour(t.id as number)
        .subscribe((result) => {
          result.forEach((r) => {
            dummyArray.push(r);
          });
  ...
  getTourKeypoints(): void {
    this.tourAuthoringService
      .getKeypointsByTour(this.tourId as number)
      .subscribe((res) => {
    ...
    if (this.tourId === 0) {
      this.tourAuthoringService.addCampaignTour(newTour).subscribe({
        next: (newTour) => {
          this.messageService.add({
            severity: 'success',
            summary: 'You have successfully saved your campaign tour',
          });
      ...
      this.tourAuthoringService.updateTour(newTour).subscribe({
        next: (updatedTour) => {
          this.messageService.add({
            severity: 'success',
            summary: 'You have successfully updated your tour',
          });
          this.newTour = updatedTour;
          this.routeQuery = {
            keypoints: this.keypoints,
