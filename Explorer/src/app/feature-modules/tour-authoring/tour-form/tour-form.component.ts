      ...
      this.tourAuthoringService.addTour(tour).subscribe({
        next: (newTour) => {
          this.messageService.add({
            severity: 'success',
            summary: 'Success',
            detail: 'You have successfully saved your tour',
          });
          this.router.navigate(['/tour-management', newTour.id]);
        },
      });
    } else {
      if (statusChange) {
        if (
          !window.confirm(
            `Are you sure that you want to ${statusChange} this tour?`,
          ) ||
        ...
        tour.statusUpdateTime = new Date();
        tour.status =
          statusChange === 'publish' ? Status.PUBLISHED : Status.ARCHIVED;
      } else {
