...
export class KeypointComponent implements OnInit {
  ...
  sendPublicEntityRequest(id: number): void {
    this.tourAuthoringService
      .getPublicEntityRequestByEntityId(id, 0)
      .subscribe({
        ...
        complete: () => {
          if (
            ...
            this.messageService.add({
              severity: 'warn',
              summary: 'Request for this keypoint already exists',
            });
          } else if (this.publicEntityRequest == null) {
            if (
              window.confirm(
                'Are you sure that you want this keypoint to be public?',
              )
            ) {
              ...
              this.tourAuthoringService
                .addPublicEntityRequestKeypoint(this.newPublicEntityRequest)
                .subscribe({
                  next: () => {
                    this.messageService.add({
                      severity: 'success',
                      summary:
                        'You have successfully sent a request for making this keypoint public',
                    });
                  },
                });
            }
