...
export class ObjectComponent implements OnInit {
  ...
  onSendRequestClicked(id: number): void {
    this.tourAuthoringService
      .getPublicEntityRequestByEntityId(id, 1)
      .subscribe({
        ...
        complete: () => {
          if (
            ...
            this.messageService.add({
              severity: 'warn',
              summary: 'Warning',
              detail: 'Request for this object already exists',
            });
          } else if (this.publicEntityRequest == null) {
            if (
              window.confirm(
                'Are you sure that you want this object to be public?',
              )
            ) {
              ...
              this.tourAuthoringService
                .addPublicEntityRequestObject(this.newPublicEntityRequest)
                .subscribe({
                  next: () => {
                    this.messageService.add({
                      severity: 'success',
                      summary: 'Success',
                      detail:
                        'You have successfully sent a request for making this object public',
                    });
                  },
                });
            }
