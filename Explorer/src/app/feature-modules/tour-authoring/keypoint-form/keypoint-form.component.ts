...
export class KeypointFormComponent implements OnChanges, OnInit {
  ...
  constructor(
    ...
    this.keypointForm = new FormGroup({
      ...
      position: new FormControl(this.keypointsCount + 1, [
        Validators.required,
        Validators.min(1),
      ]),
    });
  ...
  saveKeypoint(): void {
    if (this.tourId === 0) {
      this.messageService.add({
        severity: 'warn',
        summary: 'You can not add keypoint to undrafted tour',
      });
          ...
          next: () => {
            if (this.locationChanged) {
              ...
              this.tourAuthoringService
                .updateEncountersLocation(keypoint.id || 0, location)
                .subscribe({
                  next: () => {},
                });
            }
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Success',
              detail: 'You have successfully updated ' + keypointName,
            });
            this.keypointsUpdated.emit();
            this.keypointForm.reset();
            this.keypointForm.clearValidators();
