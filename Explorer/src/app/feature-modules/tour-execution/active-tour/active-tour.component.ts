...
import {
  TourExecutionStatus,
  TourProgress,
} from '../model/tour-progress.model';
...
import {
  ...
  EncounterStatus,
  KeypointEncounter,
} from '../../tour-authoring/model/keypointEncounter.model';
...
} from '../../encounters-managing/model/encounterCompletion.model';
...
export class ActiveTourComponent implements OnInit, OnDestroy {
    ...
    this.encounterService.startEncounter(encounter).subscribe({
      next: (result: EncounterCompletion) => {
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: 'Encounter started, go to started encounters to finish',
        });
      },
      error: (error) => {
        this.messageService.add({
      ...
      next: (encountersResult: PagedResults<Encounter>) => {
        ...
        if (encounterIds != undefined) {
          this.encounterService
            .getEncounterCompletionsByIds(encounterIds)
            .subscribe({
              next: (result: EncounterCompletion[]) => {
                encountersResult.results.forEach((encounter) => {
                  var encounterCompletion = result
                    ? result.filter((ec) => ec.encounterId === encounter.id)[0]
                    : null;
                  var encounterColor = 'yellow',
                    encounterRange = 0;
        ...
        if (result.results) {
          result.results.forEach((encounterCompletion) => {
            this.messageService.add({
              ...
              summary: 'Success',
              detail:
                'WOOO! You completed an encounter' +
                encounterCompletion.encounter.name,
            });
          });
        }
      },
  ...
  updatePosition(): void {
    if (this.requiredEncounters.length !== 0) {
      ...
      this.service.getTouristPosition().subscribe({
        next: (result: TouristPosition) => {
          if (
            this.currentPosition?.longitude !== result.longitude ||
            this.currentPosition.latitude !== result.latitude
          ) {
    ...
    this.service
      .updateActiveTour(this.requiredEncounters.length === 0)
      .pipe(
        switchMap((result: TourProgress) => {
          ...
          if (result.status === 'COMPLETED') {
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Success',
              detail: 'Tour completed at: ' + result.touristPosition?.updatedAt,
            });
            return of(null); // Return an observable to continue the chain
          }

          if (this.activeTour && this.activeTour.status === 'IN_PROGRESS') {
            this.currentKeyPoint = this.activeTour.tour.keypoints?.find(
              (keypoint) => keypoint.position === result.currentKeyPoint,
            );
            if (
              previousKeypoint != this.currentKeyPoint &&
              this.currentKeyPoint?.secret !== ''
            ) {
  ...
  getKeypointActiveEncounters(): Observable<any> {
    return this.tourAuthoringService
      .getKeypointEncounters(this.currentKeyPoint?.id || 0)
      .pipe(
        switchMap((result: PagedResults<KeypointEncounter>) => {
          this.keypointEncounters = result.results.filter(
            (item: any) => item.encounter.status === EncounterStatus.ACTIVE,
          );
  ...
  getTouristCompletedEncounters(): Observable<any> {
    return this.service.getTouristCompletedEncounters().pipe(
      switchMap((result: PagedResults<EncounterCompletion>) => {
        let completedEncounters = result.results.filter(
          (item) => item.status === 'COMPLETED',
        );
        const completedIds = completedEncounters.map((r) => r.encounter.id);
        this.requiredEncounters = this.keypointEncounters.filter(
          (item) => !completedIds.includes(item.encounterId),
        );
        this.requiredEncounters = this.requiredEncounters.filter(
    ...
    this.service.getActiveTour().subscribe({
      next: (result: TourProgress) => {
        ...
        if (this.activeTour && this.activeTour.status === 'IN_PROGRESS') {
          this.currentKeyPoint = this.activeTour.tour.keypoints?.find(
            (keypoint) =>
              keypoint.position === this.activeTour?.currentKeyPoint,
          );
        }
        this.getKeypointActiveEncounters();
      },
